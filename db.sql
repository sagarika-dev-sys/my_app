...
CREATE TRIGGER process_post_content_hashtags
 ...
 -- ==========================================
-- PART 10 — DATABASE VIEWS AND ANALYTICS
-- ==========================================

-- Operational Security View for Active Student Profiles
...
SELECT 
    ...
    s.graduation_year,
    s.is_verified,
    COUNT(p.id) FILTER (WHERE p.deleted_at IS NULL) AS total_posts_created,
...
-- Search Optimization View for Unified Global Discovery
...
SELECT 
    ...
    s.first_name || ' ' || s.last_name AS author_display_name
FROM public.posts p
...
SELECT 
    ...
    org.name AS author_display_name
FROM public.events e
JOIN public.organizations org ON e.organization_id = org.id
WHERE e.deleted_at IS NULL AND e.event_date >= CURRENT_TIMESTAMP;

-- Analytics Dashboard View for Platform Engagement Metric Intelligence
...
SELECT 
    ...
    COUNT(DISTINCT c.id) FILTER (WHERE c.deleted_at IS NULL) AS total_comments,
    COUNT(DISTINCT ph.hashtag_id) AS associated_hashtags_count,
    (COUNT(DISTINCT pl.id) * 2 + COUNT(DISTINCT c.id) FILTER (WHERE c.deleted_at IS NULL) * 5) AS weighted_engagement_score
FROM public.posts p
JOIN public.students s ON p.student_id = s.id
...
-- Trending Hashtags Monitoring and Velocity View
...
-- Comprehensive Club and Organization Event Compliance Reporting View
CREATE OR REPLACE VIEW public.view_reporting_organization_compliance AS
SELECT 
    org.id AS organization_id,
    org.name AS organization_name,
    org.type AS organization_type,
    COUNT(e.id) AS total_events_hosted,
    COUNT(e.id) FILTER (WHERE e.event_date < CURRENT_TIMESTAMP) AS past_events_count,
    COUNT(e.id) FILTER (WHERE e.event_date >= CURRENT_TIMESTAMP) AS upcoming_events_count,
    COALESCE(SUM(CASE WHEN e.event_date < CURRENT_TIMESTAMP THEN e.capacity ELSE 0 END), 0) AS cumulative_allocated_capacity,
    COUNT(er.id) FILTER (WHERE er.status = 'approved') AS total_verified_rsvps,
    CASE 
        WHEN COALESCE(SUM(CASE WHEN e.event_date < CURRENT_TIMESTAMP THEN e.capacity ELSE 0 END), 0) = 0 THEN 0.00
        ELSE ROUND((COUNT(er.id) FILTER (WHERE er.status = 'approved')::NUMERIC / SUM(CASE WHEN e.event_date < CURRENT_TIMESTAMP THEN e.capacity ELSE 0 END)::NUMERIC) * 100, 2)
    END AS average_occupancy_percentage
FROM public.organizations org
LEFT JOIN public.events e ON org.id = e.organization_id AND e.deleted_at IS NULL
LEFT JOIN public.event_requests er ON e.id = er.event_id
GROUP BY org.id;

-- Frequently Used Content Moderation and Audit Queue View
...
SELECT 
    ...
    s.is_verified AS author_verification_status,
    p.created_at AS submitted_at,
    COUNT(DISTINCT c.id) FILTER (WHERE c.deleted_at IS NULL) AS total_comments_count
FROM public.posts p
JOIN public.students s ON p.student_id = s.id
LEFT JOIN public.comments c ON p.id = c.post_id
WHERE p.deleted_at IS NULL AND (p.status = 'flagged' OR s.is_verified = FALSE)
GROUP BY p.id, s.id;

-- ==========================================
-- PART 11 — ROW LEVEL SECURITY AND POLICIES
-- ==========================================

-- Enable Row Level Security Across All Operational Tables
...
-- Profiles (Public.Students Table) Policies
CREATE POLICY "Allow public read access to active verified student profiles"
    ON public.students FOR SELECT
    USING (deleted_at IS NULL AND is_verified = TRUE);

CREATE POLICY "Allow students to update their own unique profile payload"
    ON public.students FOR UPDATE
    USING (auth.uid() = id)
    WITH CHECK (auth.uid() = id);

CREATE POLICY "Allow system administrators full access to profiles table layer"
    ON public.students FOR ALL
    USING (auth.jwt() ->> 'role' = 'admin');

-- Organizations (Clubs/Committees) Policies
CREATE POLICY "Allow anyone to view registered campus organizations"
    ON public.organizations FOR SELECT
    USING (deleted_at IS NULL);

CREATE POLICY "Allow admin to manage organization structural definitions"
    ON public.organizations FOR ALL
    USING (auth.jwt() ->> 'role' = 'admin');

-- Posts Policies
CREATE POLICY "Allow anyone to view published active system posts"
    ON public.posts FOR SELECT
    USING (status = 'published' AND deleted_at IS NULL);

CREATE POLICY "Allow authenticated students to instantiate draft posts"
    ON public.posts FOR INSERT
    WITH CHECK (auth.uid() = student_id AND status = 'draft');

CREATE POLICY "Allow authors to modify or safely drop their own content"
    ON public.posts FOR UPDATE
    USING (auth.uid() = student_id)
    WITH CHECK (auth.uid() = student_id);

CREATE POLICY "Allow admin and moderators total control over post state parameters"
    ON public.posts FOR ALL
    USING (auth.jwt() ->> 'role' = 'admin' OR auth.jwt() ->> 'role' = 'moderator');

-- Hashtags and Post-Hashtag Mapping Policies
CREATE POLICY "Allow open system discovery on global hashtags"
    ON public.hashtags FOR SELECT
    USING (TRUE);

CREATE POLICY "Allow authenticated pipelines to insert hashtag reference tokens"
    ON public.hashtags FOR INSERT
    WITH CHECK (auth.role() = 'authenticated');

CREATE POLICY "Allow open viewing of mapped post tags tracking connections"
    ON public.post_hashtags FOR SELECT
    USING (TRUE);

CREATE POLICY "Allow post authors to safely append hashtag connections"
    ON public.post_hashtags FOR INSERT
    WITH CHECK (EXISTS (
        SELECT 1 FROM public.posts 
        WHERE id = post_id AND student_id = auth.uid()
    ));

-- Comments Policies
CREATE POLICY "Allow global selection over active thread comment blocks"
    ON public.comments FOR SELECT
    USING (deleted_at IS NULL);

CREATE POLICY "Allow verified students to contribute toward open commentary"
    ON public.comments FOR INSERT
    WITH CHECK (auth.uid() = student_id);

CREATE POLICY "Allow comment authors to change or delete personal remarks"
    ON public.comments FOR UPDATE
    USING (auth.uid() = student_id)
    WITH CHECK (auth.uid() = student_id);

CREATE POLICY "Allow system moderators total deletion administration on comments"
    ON public.comments FOR DELETE
    USING (auth.jwt() ->> 'role' = 'admin' OR auth.jwt() ->> 'role' = 'moderator');

-- Post Likes Policies
CREATE POLICY "Allow transparent discovery over aggregated post liking matrices"
    ON public.post_likes FOR SELECT
    USING (TRUE);

CREATE POLICY "Allow toggle execution rules for liking records on feed posts"
    ON public.post_likes FOR INSERT
    WITH CHECK (auth.uid() = student_id);

CREATE POLICY "Allow students to revoke personal connection likes signatures"
    ON public.post_likes FOR DELETE
    USING (auth.uid() = student_id);

-- Chat Rooms Policies
CREATE POLICY "Allow security access filter over designated active rooms"
...
CREATE POLICY "Allow group room initiation structures by system authenticated users"
    ON public.chat_rooms FOR INSERT
    WITH CHECK (auth.role() = 'authenticated');

-- Chat Members Policies
CREATE POLICY "Allow participating room members to inspect peer manifest lists"
    ON public.chat_members FOR SELECT
    USING (EXISTS (
        SELECT 1 FROM public.chat_members AS internal 
        WHERE internal.room_id = room_id AND internal.student_id = auth.uid()
    ));

CREATE POLICY "Allow room initialization join parameters logic tracking"
    ON public.chat_members FOR INSERT
    WITH CHECK (auth.uid() = student_id);

-- Chat Messages Policies
CREATE POLICY "Allow stream interception viewing only to validated active members"
    ON public.chat_messages FOR SELECT
    USING (EXISTS (
        SELECT 1 FROM public.chat_members 
        WHERE room_id = chat_messages.room_id AND student_id = auth.uid()
    ));

CREATE POLICY "Enforce outbound single message verification inside authorized rooms"
...
-- Events Policies
CREATE POLICY "Allow universal read view access to operational event pipelines"
    ON public.events FOR SELECT
    USING (deleted_at IS NULL);

CREATE POLICY "Allow registered club managers creation rules across club events"
    ON public.events FOR INSERT
    WITH CHECK (EXISTS (
        SELECT 1 FROM public.organizations 
        WHERE id = organization_id AND managed_by_id = auth.uid()
    ));

CREATE POLICY "Allow organization leads complete mutations over event configurations"
...
CREATE POLICY "Allow applicants and hosts to filter single submission records"
    ...
    USING (auth.uid() = student_id OR EXISTS (
        ...
        WHERE e.id = event_id AND o.managed_by_id = auth.uid()
    ));

CREATE POLICY "Allow student identity matching request processing execution"
    ON public.event_requests FOR INSERT
    WITH CHECK (auth.uid() = student_id AND status = 'pending');

CREATE POLICY "Allow host controllers to update registration request states"
...
-- Club Posts Policies
CREATE POLICY "Allow general viewing on official organization broadcast updates"
...
-- Complaints Policies
CREATE POLICY "Allow user level self analytical view on own complaint state"
    ON public.complaints FOR SELECT
    USING (auth.uid() = student_id OR auth.jwt() ->> 'role' = 'admin' OR auth.jwt() ->> 'role' = 'moderator');

CREATE POLICY "Allow secure injection tracking metrics on ticketing layer"
    ON public.complaints FOR INSERT
    WITH CHECK (auth.uid() = student_id AND status = 'open');

CREATE POLICY "Restrict internal review operational state upgrades to authority roles"
